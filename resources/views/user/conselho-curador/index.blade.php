...
<div class="container topoConteudo">
	<h2 class="text-center mb-5 text-muted">{{$reuniao->grupo}}</h2>
	...
	@foreach ($topicos as $topico)
        		...
        		@foreach ($topicos as $topico)
					...
					<h5 class="card-title">{{$topico->ordenacao}}. {{$topico->titulo}}</h5>
						...
						@foreach ($documentos as $docs)
							...
							<ul>
								<li>
									<a href="{{route('user.conselho-curador.download', $docs->id) }}" target="_blank">{{$docs->nomedocumento}}</a>
								</li>
							</ul>
		...
		@foreach ($topicos as $topico)
					@if ($topico->categoria == 'Para Conhecimento')
				   	...
				   	@endif
		@endforeach
	<a class="btn btn-primary" title="Reuniões anteriores" href="{{ route('user.conselho-curador.pautas-anteriores')}}"><i class="fas fa-arrow-circle-left"></i> Reuniões anteriores</a>
